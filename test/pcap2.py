...
class PcapAnalyzer:
    ...
    def load_pcap(self, is_initial=True):
        ...
        try:
            ...
            if is_initial:
                self.console.print(f"[bold red]解析 PCAP 失败: {e}[/bold ed]")
                sys.exit(1)
            return False
    ...
    def exec_autocomplete(self):
        ...
        # 1. 拆分最后正在键入的单词片段（例如 'ip.s' 或 'tc'）
        # 以非字母数字及点号的字符（如空格、操作符等）作为分隔基准
        tokens = re.split(r'([^a-zA-Z0-9\._])', self.filter_text)
        last_word = tokens[-1]

        if not last_word and self.completion_idx == -1:
            return

        # 2. 状态机：如果处于连续按下 Tab 切换补全项状态中
    ...
    def generate_filter_bar(self):
        ...
        if self.is_filtering:
            ...
            border_color = "green"
            guide = "[Tab] 自动补全代码 | [Enter] 搜索并锁定 | [ESC] 取消更改"
        else:
            input_area = Text()
            ...
            guide = "[↑/↓]/[鼠标点击] 选中行 | [G] 恢复最新追尾 | [/] 过滤 | [Q] 退出"
