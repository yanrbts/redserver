...
class BlackServerProtocol(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data: bytes, addr: Tuple[str, int]):
        ...
        if len(data) > inner_tag_pos + 2 and data[inner_tag_pos:inner_tag_pos + 2] == SYMBOL:
            ...
            self.datagramReceived(data, addr)  # Log the business packet for monitoring
    ...
    def send_back_to_red(self, addr: tuple, original_data: bytes, response_json: str):
        ...
        try:
            ...
            ip_tot_len = 20 + new_udp_len          # IP头(20) + UDP报文总长
            struct.pack_into('>H', new_ip_hdr, 2, ip_tot_len)
            ...
            eth_type_hex = f"0x{orig_eth_type.hex()}"
            self.state.log_biz(
                f"SENT LENGTH: {len(resp_packet)} bytes to {addr[0]}:{self.state.port} | "
                f"ETH_TYPE: {eth_type_hex}"
            )
            # self.state.log_biz(f"SENT LENGTH: {len(resp_packet)} bytes to {addr[0]}:{self.state.port}")
        except Exception:
            pass
    ...
    def datagramReceived(self, data, addr):
        ...
        try:
            ...
            resp_packet = struct.pack('>HHII', TYPE_DATA, full_len, gw_auth, 0) + new_inner_content
            final_crc = zlib.crc32(resp_packet) & 0xFFFFFFFF
            resp_packet = struct.pack('>HHII', TYPE_DATA, full_len, gw_auth, final_crc) + new_inner_content
            
            # Push Mirror Routing Telemetry
            src_ip_str, dst_ip_str = socket.inet_ntoa(orig_src_ip), socket.inet_ntoa(orig_dst_ip)

            self.state.log_biz(f"[IN  -> ]: {src_ip_str}:{orig_src_port} -> {dst_ip_str}:{orig_dst_port} | Payload: {len(app_payload)}B")

            # self.transport.write(resp_packet, addr)
            # self.transport.sendto(resp_packet, addr)
            self.transport.sendto(resp_packet, (addr[0], self.state.port))

            # self.state.log_biz(f"[OUT -> ]: {dst_ip_str}:{orig_dst_port} -> {src_ip_str}:{orig_src_port} | Payload: {len(app_payload)}B")
            self.state.log_biz(f"[bold cyan][OUT -> ]: {dst_ip_str}:{orig_dst_port} -> {src_ip_str}:{orig_src_port} | Payload: {len(app_payload)}B[/bold cyan]")
    ...
    with Live(layout, refresh_per_second=4, screen=True):
        while state.running:
            with state.lock:
                ...
                layout["biz_box"].update(Panel(Text.from_markup("\n".join(state.biz_logs), style="green"), 
                    title="[1. Business Traffic]", border_style="green"))
