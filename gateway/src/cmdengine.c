...
static int cmd_set_auth(void *ctx, int argc, char **argv, cmd_resp_t *resp);
...
static int cmd_get_heartbeat_stats(void *ctx, int argc, char **argv, cmd_resp_t *resp);
static int cmd_pcap_filter(void *ctx, int argc, char **argv, cmd_resp_t *resp);
static int cmd_pcap_capture(void *ctx, int argc, char **argv, cmd_resp_t *resp);
...
static const cmd_entry_t cmd_table[] = {
    ...
    {NULL,  "STATUS", "show server status",               "",            NULL, 1, cmd_handle_status},
    {NULL,  "SET",    "set parameters",                   "<key> <val>", NULL, 1, cmd_handle_set},
    {NULL,  "GET",    "get parameters",                   "<key>",       NULL, 1, cmd_handle_get},
    {NULL,  "PCAP",   "packet capture ",                  "<key> <val>", NULL, 1, cmd_handle_pcap},
    {NULL,  "HELP",   "command help",                     "[command]",   NULL, 1, cmd_handle_help},
    {NULL,  "EXIT",   "exit the CLI",                     "",            NULL, 1, NULL},
    /* --- SET Group Sub-commands --- */
    {"SET", "interval", "Scan interval (ms)",              "<val>",      NULL, 3, NULL},
    {"SET", "logpkt",   "print raw XDP packet info",       "<1|0>",      NULL, 3, cmd_set_islogptk},
    {"SET", "debug",    "enable/disable debug mode",       "<1|0>",      NULL, 3, cmd_set_isdebug},
    {"SET", "auth",     "set authentication request",       "",          NULL, 2, cmd_set_auth},
    /* --- GET Group Sub-commands --- */
    {"GET", "pktstats", "get packet statistics",           "",           NULL, 2, cmd_get_reass_stats},
    {"GET", "config",   "get configuration",               "",           NULL, 2, cmd_get_config},
    {"GET", "heartbeat", "get heartbeat statistics",       "",           NULL, 2, cmd_get_heartbeat_stats},
    /* --- PCAP Group Sub-commands --- */
    {"PCAP", "filter",  "set BPF filter of packet",        "<eth1|eth2> <expr>",        
        "<expr> Example IP: \"host 192.168.1.100\"\n"
        "<expr> Example TCP: \"tcp port 80\"\n"
        "<expr> Example Filter: \"dst host 192.168.211.130 and tcp port 80\"\n"
        "<expr> Example Exclude: \"not host 127.0.0.1\"\n", 
        4, 
        cmd_pcap_filter
    },
    {"PCAP", "capture", "set packet capture",              "<eth1|eth2> <on/1|off/0> <expr>",  
        "<expr> Example IP: \"host 192.168.1.100\"\n"
        "<expr> Example TCP: \"tcp port 80\"\n"
        "<expr> Example Filter: \"dst host 192.168.211.130 and tcp port 80\"\n"
        "<expr> Example Exclude: \"not host 127.0.0.1\"\n", 
...
static int cmd_set_auth(void *ctx, int argc, char **argv, cmd_resp_t *resp) {
    ...
    //     cmd_resp_red(resp, "ERR: Missing parameter. Usage: SET auth");
    //     return -1;
    // }

    // char *auth[] = {"auth", "code", "request"};
    // auth_cmd(auth, 3);
    // cmd_resp_green(resp, "OK: Authentication request sent successfully.");

    // log_info("[MGMT] Configuration updated: Authentication request sent by administrator.");
    return 0;
}

/**
 * @brief Generates a standardized PCAP file path prefixed with the current timestamp.
 * This function constructs a deterministic, thread-safe filename utilizing the ISO 8601-like
 * compressed format (YYYYMMDD_HHMMSS) to prevent collision and preserve timeline indexing.
 * Anti-buffer-overflow countermeasures are enforced via boundary telemetry.
 * @param[out] out_buf   Pointer to the destination char buffer where the path will be written.
...
static inline bool cmd_pcapfile_path(char *out_buf, size_t max_len) {
    ...
    // Utilize reentrant version 'localtime_r' to ensure multi-threading reentrancy safety
    if (!localtime_r(&raw_time, &time_info)) {
        return false;
    }

    // Format target metrics into an immutable timestamp template: YYYYMMDD_HHMMSS
    // Total footprint of formatted string is 15 bytes plus null terminator
    if (strftime(time_str, sizeof(time_str), "%Y%m%d_%H%M%S", &time_info) == 0) {
        return false;
    }
...
static inline void cmd_pcap_init(const char *eth) {
...
static int cmd_pcap_filter(void *ctx, int argc, char **argv, cmd_resp_t *resp) {
    ...
    if (argc < 4 || argv[2] == NULL || argv[3] == NULL) {
        cmd_resp_red(resp, "ERR: Missing parameter. Usage: PCAP filter <eth1|eth2> <BPF expression>");
        return -1;
    }
    ...
    cmd_pcap_init(argv[2]);

    const char *filter_expr = argv[3];
    ...
    if (pcap_mod_set(engine->filter_expr) < 0) {
        cmd_resp_red(resp, 
                    ...
                    "• DNS         : udp port 53\n"
                    "• Exclude filter: dst host 192.168.211.130 and tcp port 80\n"
...
static int cmd_pcap_capture(void *ctx, int argc, char **argv, cmd_resp_t *resp) {
    ...
    if (argc < 4 || argv[2] == NULL || argv[3] == NULL) {
        cmd_resp_red(resp, "ERR: Missing parameter. Usage: PCAP capture <eth1|eth2> <on|off>");
        return -1;
    }
    ...
    if (strcmp(val_str, "1") == 0 || 
        ...
        cmd_resp_red(resp, "ERR: Invalid value '%s'. Expected: on or off.", val_str);
        return -1;
    }
    ...
    atomic_store(&engine->ispcap, enable);
    cmd_resp_green(resp, "OK: Packet capture %s %s.", is_normal_stop ? "" : "already", enable ? "STARTED" : "STOPPED");
    log_info("[MGMT] Configuration updated: Packet capture %s by administrator.", enable ? "STARTED" : "STOPPED");
...
static int cmd_get_heartbeat_stats(void *ctx, int argc, char **argv, cmd_resp_t *resp) {
    ...
    // if (argc != 2) {
    //     cmd_resp_red(resp, "ERR: Unexpected arguments. Usage: GET heartbeat");
    //     return -1;
    // }

    // seek_mgr_get_debug_status(mgr, &engine->heart_stats);

    // uint64_t active_ports = engine->heart_stats.active_ports;
    // // uint64_t discovery_count = engine->heart_stats.discovery_count;
    // uint64_t register_count = engine->heart_stats.register_count;
    // uint64_t heartbeat_count = engine->heart_stats.heartbeat_count;

    // cmd_resp_printf(resp, "\n  %s%-20s%s : %s%s%s\n", C_GREEN, "Discovery status", C_RESET, C_YELLOW, active_ports ? "yes" : "no", C_RESET);
    // // cmd_resp_printf(resp, "  %s%-20s%s : %s%s%s\n", C_GREEN, "Discovery status", C_RESET, C_YELLOW, discovery_count ? "yes" : "no", C_RESET);
    // cmd_resp_printf(resp, "  %s%-20s%s : %s%s%s\n", C_GREEN, "Register status", C_RESET, C_YELLOW, register_count ? "yes" : "no", C_RESET);
    // cmd_resp_printf(resp, "  %s%-20s%s : %s%s%s\n", C_GREEN, "Heartbeat status", C_RESET, C_YELLOW, heartbeat_count ? "yes" : "no", C_RESET);
