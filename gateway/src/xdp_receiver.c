...
static int handle_ringbuf_sample(void *ctx, void *data, size_t data_sz) {
    ...
    if (ictx->user_cb) {
        return ictx->user_cb(ictx->user_ctx, e->data, e->pkt_len, e->ifindex);
    } else {
        pkt_info_t info = {0};
...
void* xdp_receiver_init(const xdp_receiver_config_t *cfg, xdp_packet_cb cb) {
    if (!cfg || !cfg->bpf_obj_path || !cfg->ifname_A || !cfg->ifname_B) {
        log_error("Initialization aborted: Missing configuration variables or interface specs");
        return NULL;
    }
    ...
    /* Turn network interface labels into kernel-space hardware IDs */
    ictx->ifindex_A = (int)if_nametoindex(cfg->ifname_A);
    ictx->ifindex_B = (int)if_nametoindex(cfg->ifname_B);
    if (ictx->ifindex_A == 0 || ictx->ifindex_B == 0) {
        log_error("Failed to map hardware interface labels to valid system ifindices");
        goto cleanup;
    }
    ...
    if (ictx->ifindex_A == ictx->ifindex_B) {
        ...
        if (ictx->verbose) {
            log_info("System detected Single-NIC Topo. Operating exclusively on boundary interface: %s (%d)", 
        ...
        if (bpf_xdp_attach(ictx->ifindex_B, prog_fd, XDP_FLAGS_SKB_MODE, NULL) < 0) {
            log_error("Failed to attach XDP capture hook to Interface B (%s)", cfg->ifname_B);
            bpf_xdp_detach(ictx->ifindex_A, XDP_FLAGS_SKB_MODE, NULL); // 回滚
            goto cleanup;
        }
        if (ictx->verbose) {
            log_info("System detected Dual-NIC Topo. Attached to: [A: %s] <-> [B: %s]", cfg->ifname_A, cfg->ifname_B);
        }
    }
