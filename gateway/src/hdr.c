...
int hdr_verify_crc(const unsigned char *raw_hdr, ssize_t raw_len) {
    /* Boundary check */
    if (!raw_hdr || raw_len < (ssize_t)HDR_SIZE) {
        return 0;
    }

    /* Bypass read-only restriction for inplace modification */
    unsigned char *p = (unsigned char *)raw_hdr;

    /* Stash original 4-byte CRC from wire */
    uint8_t crc_bak[4];
    crc_bak[0] = p[8];
    crc_bak[1] = p[9];
    crc_bak[2] = p[10];
    ...
    /* Zero out CRC field inline (equivalent to clearing it via temp buffer) */
    p[8]  = 0;
    p[9]  = 0;
    p[10] = 0;
    p[11] = 0;

    /* Execute native calculation over the intact frame */
    uint32_t calc_crc = crc32_manual(raw_hdr, (size_t)raw_len);

    /* Restore wire memory state immediately after calculation */
    p[8]  = crc_bak[0];
    p[9]  = crc_bak[1];
    p[10] = crc_bak[2];
    p[11] = crc_bak[3];

    /* Parse stashed big-endian CRC into host order */
    uint32_t stored_crc = ((uint32_t)crc_bak[0] << 24) | 
                          ((uint32_t)crc_bak[1] << 16) | 
                          ((uint32_t)crc_bak[2] << 8)  | 
                           (uint32_t)crc_bak[3];
