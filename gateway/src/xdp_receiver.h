 ...
 */
struct packet_event {
    uint32_t ifindex;         /**< Ingress network interface ID (e.g., card A or card B) */
...
typedef struct xdp_receiver_config {
    ...
    const char *ifname_A;       /**< Interface A name connected to the Black Zone */
    const char *ifname_B;       /**< Interface B name connected to the Client Switch */
 ...
 * @param ingress_ifindex The interface ID where this packet originated.
 * @return 0 to continue, negative value to signal a stop to the receiver.
 */
typedef int (*xdp_packet_cb)(void *user_ctx, const uint8_t *pkt, size_t pkt_len, uint32_t ingress_ifindex);
