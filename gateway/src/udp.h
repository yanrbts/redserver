...
/**
 * @brief Binds the UDP socket to a specific network interface.
 * This is useful for multi-homed hosts or when you want to ensure that
 * traffic goes out through a specific interface. The interface is specified by name (e.g.,
 * "eth0", "wwan0"). This function uses the SO_BINDTODEVICE socket option.
 * @param conn The socket handle descriptor.
 * @param if_name The name of the network interface to bind to.
 * @return 0 on success, -1 on failure.
 * @warning Requires appropriate permissions (CAP_NET_ADMIN) to bind to an interface.
 */
