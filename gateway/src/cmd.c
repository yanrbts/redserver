...
void cmd_resp_gray(cmd_resp_t *r, const char *fmt, ...) {
    if (!r || !fmt) return;

    cmd_resp_print_raw(r, "%s", C_GRAY);
...
void cmd_resp_orange(cmd_resp_t *r, const char *fmt, ...) {
    if (!r || !fmt) return;

    cmd_resp_print_raw(r, "%s", C_ORANGE);
...
/**
 * @brief Utility to print multi-line examples with responsive tree-like padding.
 * This encapsulates the zero-allocation cursor slicing logic for sub-command examples.
 * @param[in,out] resp           The response container/buffer descriptor.
 * @param[in]     example_str    The raw example string (supporting embedded '\n').
 * @param[in]     col1_w         Width metric assigned to Column 1.
 * @param[in]     col2_w         Width metric assigned to Column 2.
 */
static void cmd_resp_print_example(cmd_resp_t *resp, const char *example_str, int col1_w, int col2_w) {
    // Defensive gate: prevent segmentation faults on dynamic/uninitialized table tracks
    if (!resp || !example_str || example_str[0] == '\0') {
        return;
    }

    // Dynamic layout offset estimation
    int example_indent = col1_w + 1 + col2_w + 1 + 2;

    const char *start = example_str;
    const char *end;

    // High-performance pointer-sliding window matrix
    while (*start != '\0') {
        end = start;
        while (*end != '\n' && *end != '\0') {
            end++;
        ...
        if (line_len > 0) {
            // Secure bounded length format execution (via precision parameter '.*')
            cmd_resp_orange(resp, "%*s• %.*s\n", 
                          example_indent, "", 
                          (int)line_len, start);
        }
    ...
    for (int i = 0; g_table[i].name != NULL; i++) {
        ...
        if (match) {
            ...
            cmd_resp_green(resp, "%-*s %-*s %s\n",
                            ...
                            usage_str);
            cmd_resp_print_example(resp, g_table[i].example, COL1_WIDTH, COL2_WIDTH);
            ...
            if (group == NULL) {
                for (int j = 0; g_table[j].name != NULL; j++) {
                    if (g_table[j].group != NULL && strcasecmp(g_table[j].group, g_table[i].name) == 0) {
                        ...
                        cmd_resp_print_example(resp, g_table[j].example, COL1_WIDTH, COL2_WIDTH);
                    }
                }
            }
        }
